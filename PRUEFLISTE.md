...
| Spiegel im Formular mit eigenen Ressourcen ohne `/Properties`, Name nur beim Aufrufer auflösbar (Poppler); überschattete gleichnamige Liste beim Aufrufer | A | Spur-A-Runde 1 (#67), Runde 2 (#86: Eintrag `null`; #85: Annotationserscheinung; #87: überschattete Kopie) | `crates/redact-pdf/tests/zo_c_spiegel_umgebungen.rs` |
...
| Beiwerk mit Klartext an Katalog, Seite, Objekt und Signaturfeld (`/Perms`, `/DSS`, `/PageLabels`, `/Threads`, `/Collection`, `/OutputIntents`, `/URI`, `/DPartRoot`, `/VP`, `/PresSteps`, `/Ref`, `/OPI`, `/SV`, `/Lock`); `/PageLabels` nach einem direkten Wert und in der Tiefe; jeder Schlüssel an Katalog, Seitenbaum und Seite außerhalb der Erlaubnisliste (`/SpiderInfo`, `/Legal`, `/Requirements`), Seiten-`/OutputIntents`, `/3DU`; ein fremder Schlüssel in einem Ressourcenverzeichnis | A | Spur-A-Runde 1 (#70), Spur-A-Runde 2 (#92, #107) | `crates/redact-pdf/tests/zo_b_traeger.rs`, `crates/redact-pdf/tests/zp_b_seitenbeschriftung.rs`, `crates/redact-pdf/tests/zp_b_erlaubte_schluessel.rs`, `crates/redact-pdf/tests/zp_b_fremde_ressourcen.rs` |
| Vervielfachung über Seiten: ein `/Annots`, ein Inhaltsstrom oder ein Formular, das sich viele Seiten teilen | A | Spur-A-Runde 2 (#94, #106) | `crates/redact-pdf/tests/zp_b_geteiltes_annots.rs`, `crates/redact-pdf/tests/zp_b_geteilter_inhalt.rs` |
...
| Erscheinungsstrom, den niemand liest: `/MK /I`, `/RI`, `/IX`; `/AP` einer nur über `/Popup`, `/Parent`, `/IRT` gehaltenen Annotation; `/AP` eines Widgets, das keine Seite zeigt, oder am Ende einer langen Antwortkette | A | Spur-A-Runde 1 (#71), Spur-A-Runde 2 (#90, #91) | `crates/redact-pdf/tests/zo_b_traeger.rs`, `crates/redact-pdf/tests/zp_b_nicht_gezeichnete_annotation.rs` |
...
| Orakel: eine Zeichenkettenkodierung der Norm, die der Dekoder nicht kennt (PDFDocEncoding 0x80–0xA0, UTF-8 mit BOM, WinAnsi im Inhaltsstrom) | A | Spur-A-Runde 1 (#81), Spur-A-Runde 2 (#97) | `crates/redact-pdf/tests/zo_d_altgeneration_und_kodierung.rs`, `crates/redact-cli/tests/zo_d_orakel_am_binary.rs`, `crates/redact-pdf/tests/zp_d_bom_und_winansi.rs` |
| Orakel: eine Altgeneration unter einem Filter, den nur die Objektsicht kann, oder als maskierte Zeichenkette; ihr Dictionary in kompakter Schreibweise oder mit Verweis, eine Kette, die die Rohsicht nicht dekodiert | A | Spur-A-Runde 1 (#80), Spur-A-Runde 2 (#95) | `crates/redact-pdf/tests/zo_d_altgeneration_und_kodierung.rs`, `crates/redact-cli/tests/zo_d_orakel_am_binary.rs`, `crates/redact-pdf/tests/zp_d_rohes_dictionary.rs` |
